...
beforeEach(() => {
  ...
  wrapper = rtl.render(
    <Provider store={store}>
      <Main isFetching={false} photo={mockPhoto} />
    </Provider>
  );
});
// Make a trivial test
...
//   expect(wrapper.queryByText("Next")).toBeInTheDocument();
//   expect(wrapper.queryByText("Next")).toBeVisible();
// });

// it('renders a "Set as Favorite" text node', () => {
//   expect(wrapper.queryByText("Set as Favorite")).toBeInTheDocument();
//   expect(wrapper.queryByText("Set as Favorite")).toBeVisible();
// });
